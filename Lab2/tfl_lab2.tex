...
\begin{enumerate}
\item Язык, в котором слово либо пустое, либо начинается на aa или bb.
...
\begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto]
    ...
    \node[state] (T) [right=8 of p0] {$T$};
    \node[state, accepting] (p1) [above right=2 of p0] {$p_1$};
    % Второй автомат
    ...
    \path[->]
        (p0) edge [bend left=20] [above]node {a} (p1)
        (p0) edge node {b} (T)
        (p1) edge [loop right] node {a,b} ()
    ...
    \path[->]
        ...
        (q4) edge [bend right=70] [above] node {a} (q0)
        (q4) edge [bend right] [below] node {b} (T)
\end{tikzpicture}
\end{center}
\textbf{Описание автомата:}
\begin{itemize}
    \item Начальное состояние: $\&$
    \item Конечные состояния: $\&$, $p_1$, $q_0$, $q_3$
    \item Алфавит: $\{a, b\}$
\end{itemize}
\textbf{Таблица множеств классов эквивалентности:}
\begin{center}
